...
(
    ...
    statut            VARCHAR(30) DEFAULT 'Recue' CHECK (statut IN ('Recue', 'Preselectionnee', 'Entretien', 'Acceptee',
                                                                    'Refusee', 'En attente')),
...
(
    ...
    commentaire        TEXT,
    evaluateur         VARCHAR(100),
    recommandation     VARCHAR(50) CHECK (recommandation IN
                                          ('Promotion', 'Formation', 'Maintien', 'Avertissement', 'Licenciement'))
);
...
(
    ...
    id_employe   INTEGER     NOT NULL REFERENCES Employe (id_employe),
    type_conge   VARCHAR(50) NOT NULL CHECK (type_conge IN ('Annuel', 'Maladie', 'Maternite', 'Paternite', 'Sans solde',
                                                            'Exceptionnel', 'Formation')),
...
(
    ...
    id_employe    INTEGER     NOT NULL REFERENCES Employe (id_employe),
    jour_semaine  VARCHAR(10) NOT NULL CHECK (jour_semaine IN
...
(
    ...
    type_deduction VARCHAR(100)   NOT NULL CHECK (type_deduction IN
                                                  ('IRSA', 'CNaPS', 'OSTIE', 'Mutuelle', 'Avance', 'Autre')),
    description    TEXT,
...
-- ============================================================
-- TABLE CONFIGURATION — APPLICATION RH MADAGASCAR
-- Paramètres légaux et réglementaires (cotisations, IRSA, congés...)
-- ============================================================

-- ============================================================
-- 1. TABLE CONFIGURATION (paramètres clé-valeur par unité)
-- ============================================================
CREATE TABLE Configuration
(
    id_config     SERIAL PRIMARY KEY,
    unite         INTEGER      NOT NULL,        -- Numéro d'unité fonctionnelle (1 à 6)
    nom_unite     VARCHAR(100) NOT NULL,        -- Libellé de l'unité
    cle           VARCHAR(100) NOT NULL UNIQUE, -- Clé du paramètre (ex: TAUX_CNAPS_EMPLOYE)
    valeur        VARCHAR(100) NOT NULL,        -- Valeur du paramètre (stockée en texte)
    type_valeur   VARCHAR(20)  NOT NULL CHECK (type_valeur IN
                                               ('POURCENTAGE', 'MONTANT', 'HEURES', 'JOURS', 'HEURE', 'ANNEES',
                                                'TEXTE')),
    description   TEXT,
    actif         BOOLEAN   DEFAULT TRUE,
    date_creation DATE      DEFAULT CURRENT_DATE,
...
-- ============================================================
-- 2. TABLE BAREME IRSA (tranches progressives)
-- ============================================================
CREATE TABLE BaremeIRSA
(
    id_tranche     SERIAL PRIMARY KEY,
    numero_tranche INTEGER        NOT NULL,
    revenu_min     NUMERIC(15, 2) NOT NULL, -- Borne inférieure de la tranche
    revenu_max     NUMERIC(15, 2),          -- NULL = illimité (dernière tranche)
    taux           NUMERIC(5, 2)  NOT NULL, -- Taux en %
    description    VARCHAR(100),
    actif          BOOLEAN DEFAULT TRUE,
    annee_validite INTEGER DEFAULT 2025
);

-- ============================================================
-- 3. INDEX
-- ============================================================
CREATE INDEX idx_config_unite ON Configuration (unite);
CREATE INDEX idx_config_cle ON Configuration (cle);
CREATE INDEX idx_irsa_tranche ON BaremeIRSA (numero_tranche);
